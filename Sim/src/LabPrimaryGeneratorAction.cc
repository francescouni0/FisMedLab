...
LabPrimaryGeneratorAction::LabPrimaryGeneratorAction(LabInfo *info_)
 : G4VUserPrimaryGeneratorAction(), fParticleGun1(0){
  ...
  fParticleGun1->SetParticleMomentum(0*GeV);
...
//   }
  ...
  // for output
  info->particleName = fParticleGun1->GetParticleDefinition()->GetParticleName();
  info->beamEnergy   = fParticleGun1->GetParticleEnergy()/keV;
  info->beamPos[0]   = fParticleGun1->GetParticlePosition().x()/mm;
  info->beamPos[1]   = fParticleGun1->GetParticlePosition().y()/mm;
  info->beamPos[2]   = fParticleGun1->GetParticlePosition().z()/mm;
  info->beamDirection[0]   = fParticleGun1->GetParticleMomentumDirection().x();
  info->beamDirection[1]   = fParticleGun1->GetParticleMomentumDirection().y();
  info->beamDirection[2]   = fParticleGun1->GetParticleMomentumDirection().z();
