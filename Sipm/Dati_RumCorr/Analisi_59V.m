...
outlier2 = ampl>((0.00186505.*time_diff)+0.1);
outliers = outlier1|outlier2;
% fit_func1 = @(a, b, c, x) -a*exp(-b.*x)+c;
% ampl_fit = fit(time_diff, ampl, fit_func1, 'Exclude', outliers, 'Start', [0.11 0.07 10])
...
% hold on
% plot(time_diff, ((0.00186505.*time_diff)+0.1))
% hold off
...
ylabel('Occurrence')
ylim([1 1e3])
sgtitle('Correlated noise, OV=6.33V')
...
ylabel('Amplitude [a.u.]')
ylim([min(ampl) 0.75])
yl1 = yline(0.295, '-', 'First Threshold');
yl2 = yline(0.25, '-', 'Second Threshold');
...
ampl1 = ampl1(ampl1>0.295);
ampl2 = ampl2(ampl2>0.25);
