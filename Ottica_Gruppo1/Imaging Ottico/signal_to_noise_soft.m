function [SNR, error_SNR] = signal_to_noise_soft(image, dark, binning)

hard = importdata(image);
hard = hard(:,2:end);

hard_noise = importdata(dark);
hard_noise = hard_noise(:,2:end);

im = soft_binning(hard, binning);

noise = soft_binning(hard_noise, binning);
